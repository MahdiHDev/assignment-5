...
var import_prisma3 = require("better-auth/adapters/prisma");
...
var auth = (0, import_better_auth.betterAuth)({
  database: (0, import_prisma3.prismaAdapter)(prisma, {
    provider: "postgresql"
...
var registerUserSchema = import_zod2.z.object({
  name: import_zod2.z.string({ error: "Name is required" }).min(2, "Name must be at least 2 characters").max(100, "Name must not exceed 100 characters").trim(),
  email: import_zod2.z.string({ error: "Email is required" }).email("Invalid email address").trim().toLowerCase(),
  password: import_zod2.z.string({ error: "Password is required" }).min(8, "Password must be at least 8 characters").max(128, "Password must not exceed 128 characters")
});
var loginUserSchema = import_zod2.z.object({
  email: import_zod2.z.string({ error: "Email is required" }).email("Invalid email address").trim().toLowerCase(),
  password: import_zod2.z.string({ error: "Password is required" }).min(1, "Password is required")
});
var changePasswordSchema = import_zod2.z.object({
  currentPassword: import_zod2.z.string({ error: "Current password is required" }).min(1, "Current password is required"),
  newPassword: import_zod2.z.string({ error: "New password is required" }).min(8, "New password must be at least 8 characters").max(128, "New password must not exceed 128 characters")
...
var verifyEmailSchema = import_zod2.z.object({
  email: import_zod2.z.string({ error: "Email is required" }).email("Invalid email address").trim().toLowerCase(),
  otp: import_zod2.z.string({ error: "OTP is required" }).length(6, "OTP must be 6 digits").regex(/^\d+$/, "OTP must contain only digits")
});
...
var slotSchema = import_zod3.z.object({
  dayOfWeek: import_zod3.z.enum(DayOfWeek, {
    error: `dayOfWeek must be one of: ${Object.values(DayOfWeek).join(", ")}`
  }),
  startTime: import_zod3.z.string().regex(
    timeRegex,
    "Invalid time format. Use HH:MM (24-hour format)"
  ),
  endTime: import_zod3.z.string().regex(
    timeRegex,
    "Invalid time format. Use HH:MM (24-hour format)"
  )
...
var createAvailabilitySchema = import_zod3.z.object({
  startDate: import_zod3.z.coerce.date(),
  endDate: import_zod3.z.coerce.date(),
  slots: import_zod3.z.array(slotSchema).min(1, "At least one slot is required")
}).refine((data) => data.startDate < data.endDate, {
  message: "Invalid date range",
  path: ["endDate"]
});
var updateAvailabilitySchema = import_zod3.z.object({
  dayOfWeek: import_zod3.z.enum(DayOfWeek, {
    error: `dayOfWeek must be one of: ${Object.values(DayOfWeek).join(", ")}`
  }).optional(),
  startTime: import_zod3.z.string().regex(timeRegex, "Invalid time format. Use HH:MM (24-hour format)").optional(),
  endTime: import_zod3.z.string().regex(timeRegex, "Invalid time format. Use HH:MM (24-hour format)").optional(),
...
// src/modules/availability/availability.routes.ts
var availabilityRoutes = (0, import_express4.Router)();
...
var createBookingSchema = import_zod4.z.object({
  sessionDate: import_zod4.z.iso.date({
    error: (issue) => issue.input === void 0 ? "Session date is required" : "sessionDate must be a valid date (YYYY-MM-DD)"
  }),
  startTime: import_zod4.z.string({ error: "Start time is required" }).regex(timeRegex2, "Invalid time format. Use HH:MM"),
  endTime: import_zod4.z.string({ error: "End time is required" }).regex(timeRegex2, "Invalid time format. Use HH:MM"),
  tutorCategoryId: import_zod4.z.string({ error: "Tutor category is required" }).min(1, "Tutor category is required")
...
var bookingStatusSchema = import_zod4.z.object({
  status: import_zod4.z.enum(BookingStatus, {
    error: `Invalid status. Status must be one of: ${Object.values(BookingStatus).join(", ")}`
  }),
  meetingLink: import_zod4.z.url("meetingLink must be a valid URL").optional()
});
...
// src/modules/booking/booking.routes.ts
var bookingRoutes = (0, import_express5.Router)();
bookingRoutes.post(
  "/create",
...
var router2 = (0, import_express6.Router)();
router2.post(
  "/checkout/:bookingId",
...
var createReviewSchema = import_zod5.z.object({
  bookingId: import_zod5.z.string().min(1, "bookingId is required"),
  rating: import_zod5.z.number({ error: "Rating is required" }).int("Rating must be a whole number").min(1, "Rating must be between 1 and 5").max(5, "Rating must be between 1 and 5"),
  comment: import_zod5.z.string().max(1e3, "Comment must be under 1000 characters").optional()
});
...
// src/modules/review/review.routes.ts
var reviewRoutes = (0, import_express7.Router)();
reviewRoutes.post(
  "/create",
  checkAuth("STUDENT" /* STUDENT */, "TUTOR" /* TUTOR */, "ADMIN" /* ADMIN */),
  checkBanStatus_default,
...
// src/modules/subject/subject.routes.ts
var subjectRoutes = (0, import_express8.Router)();
subjectRoutes.post(
  "/create",
  checkAuth("ADMIN" /* ADMIN */),
...
var createTeachingSessionSchema = import_zod6.z.object({
  subjectName: import_zod6.z.string({ error: "Subject name is required" }).trim().min(1, "Subject name is required").max(100, "Subject name must be under 100 characters"),
  hourlyRate: import_zod6.z.number({ error: "Hourly rate is required" }).positive("Hourly rate must be greater than 0"),
  experienceYears: import_zod6.z.number({ error: "Experience years is required" }).int("Experience years must be a whole number").min(0, "Experience years cannot be negative"),
  level: import_zod6.z.enum(TutorLevel, {
    error: `Level must be one of: ${Object.values(TutorLevel).join(", ")}`
  }),
  bio: import_zod6.z.string().max(2e3, "Bio must be under 2000 characters").optional(),
  isPrimary: import_zod6.z.boolean().optional()
});
var updateTeachingSessionSchema = import_zod6.z.object({
  hourlyRate: import_zod6.z.number().positive("Hourly rate must be greater than 0").optional(),
  experienceYears: import_zod6.z.number().int("Experience years must be a whole number").min(0, "Experience years cannot be negative").optional(),
  level: import_zod6.z.enum(TutorLevel, {
    error: `Level must be one of: ${Object.values(TutorLevel).join(", ")}`
  }).optional(),
  description: import_zod6.z.string().max(2e3, "Description must be under 2000 characters").optional(),
  isPrimary: import_zod6.z.boolean().optional()
...
// src/modules/tutor/tutor.routes.ts
var tutorRoutes = (0, import_express9.Router)();
